...
def health_check():
	...
	for i in range(0,len(containers)):
		...
		if resp.status_code==200:
			...
			DOCKER_START="docker run --rm --name "+str(containers[i])+" -d -p "+str(containers[i])+":80 -ti -v a6:/file acts"
...
DOCKER_START="docker  run --rm --name 8000  -p 8000:80 -ti -v a6:/file acts"
...
def add_con(add_con):
	...
	for i in range(add_con):
		...
		DOCKER_START="docker  run --rm --name "+str(new_port)+" -d -p "+str(new_port)+":80 -ti -v a6:/file acts"
		os.system(DOCKER_START)
...
def catch_all(path):
	...
	if(count ==1 and one_flag):
		...
		scheduler.add_job(new_container, 'interval', seconds = 120)
